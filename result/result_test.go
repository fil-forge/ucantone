...
func TestMatchResult(t *testing.T) {
	...
	t.Run("MatchResultR2", func(t *testing.T) {
		...
		testMatchResultR3(t, "err (error)",
			result.Error[int](errors.New("bad")),
			func(o int) (string, int, error) { return "", 0, nil },
			func(x error) (string, int, error) { return x.Error(), len(x.Error()), fmt.Errorf("something: %w", x) },
			"bad", 3, fmt.Errorf("something: %w", errors.New("bad")))
	})
}
