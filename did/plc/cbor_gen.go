...
func (t *Operation) MarshalCBOR(w io.Writer) error {
	...
	if t.Previous == nil {
		...
		if len(*t.Previous) > 8192 {
			return xerrors.Errorf("Value in field t.Previous was too long")
		}

		if err := cw.WriteMajorTypeHeader(cbg.MajTextString, uint64(len(*t.Previous))); err != nil {
			return err
		}
		if _, err := cw.WriteString(string(*t.Previous)); err != nil {
			return err
		}
	}
...
func (t *Operation) UnmarshalCBOR(r io.Reader) (err error) {
	*t = Operation{}
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("Operation: map struct too large (%d)", extra)
	}
	...
	for i := uint64(0); i < n; i++ {
		...
		case "rotationKeys":
			...
			if extra > 0 {
				t.RotationKeys = make([]did.DID, extra)
			}
			...
			for i := 0; i < int(extra); i++ {
				{
...

					{
						...
						if err := t.RotationKeys[i].UnmarshalCBOR(cr); err != nil {
							return xerrors.Errorf("unmarshaling t.RotationKeys[i]: %w", err)
						}
		...
		case "verificationMethods":
			...
			t.VerificationMethods = make(map[string]did.DID, extra)
...
func (t *SignedOperation) MarshalCBOR(w io.Writer) error {
	...
	if _, err := cw.WriteString(string(t.Signature)); err != nil {
		return err
	}
...
func (t *SignedOperation) UnmarshalCBOR(r io.Reader) (err error) {
	*t = SignedOperation{}
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("SignedOperation: map struct too large (%d)", extra)
	}
...
func (t *SignedTombstone) UnmarshalCBOR(r io.Reader) (err error) {
	*t = SignedTombstone{}
	...
	if extra > cbg.MaxLength {
		return fmt.Errorf("SignedTombstone: map struct too large (%d)", extra)
	}
